...
func (ts *Thresholds) Run(sink Sink, duration time.Duration) (bool, error) {
	...
	// see: https://github.com/grafana/k6/pull/2251#discussion_r769807203
	//
	...
	//
	// For more details, see https://github.com/grafana/k6/pull/2251#discussion_r769807203
	...
	case *TrendSink:
		...
		for _, threshold := range ts.Thresholds {
			...
			// Percentile value has already been parsed as a float64, and should be valid.
			// The error check is only here to protect our future selves.
			percentile, err := strconv.ParseFloat(trimDelimited("p(", threshold.parsed.AggregationMethod, ")"), 64)
			if err != nil {
				return false, fmt.Errorf(
					"unable to parse percentile value as 64-bit floating point number; "+
						"reason: %w", err,
				)
			}

			ts.sinked[threshold.parsed.AggregationMethod] = sinkImpl.P(percentile / 100)
		}
	case *RateSink:
