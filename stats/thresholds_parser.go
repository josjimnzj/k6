...
func parseThresholdExpression(input string) (*thresholdExpression, error) {
	...
	parsedMethod, err := parseThresholdAggregationMethod(method)
	if err != nil {
...
func parseThresholdAggregationMethod(input string) (string, error) {
	...
	if strings.HasPrefix(input, "p(") && strings.HasSuffix(input, ")") {
		_, err := strconv.ParseFloat(trimDelimited("p(", input, ")"), 64)
		if err != nil {
			return "", fmt.Errorf("malformed percentile value; reason: %w", err)
		}

		return input, nil
	}

	return "", fmt.Errorf("failed parsing method from expression")
}
